  ...
  return (
    <div className="property-page-root">
      ...
      <nav className="prop-nav">
        <Link href="/" style={{ fontWeight: '800', color: 'white', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '10px' }} className="prop-nav-logo-text">
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <Link href="/propiedades" style={{ color: '#cbd5e1', textDecoration: 'none', fontSize: '14px', fontWeight: '600' }}>
            ← Catálogo
          </Link>
          <a 
            href={whatsappUrl} 
      ...
      <div style={{ background: '#ffffff', borderBottom: '1px solid #e2e8f0', padding: '20px 16px' }}>
          ...
          <div className="prop-header-flex">
            <div>
              <div style={{ display: 'flex', gap: '6px', marginBottom: '8px', flexWrap: 'wrap' }}>
                <span style={{ background: '#0284c7', color: 'white', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold' }}>
                  {inmueble.operacion}
                </span>
                <span style={{ background: '#e2e8f0', color: '#334155', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold' }}>
                  {inmueble.tipo}
                </span>
                {inmueble.licencia_turistica && (
                  <span style={{ background: '#10b981', color: 'white', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold' }}>
                    Licencia Turística
                  </span>
                )}
                {inmueble.etapa && (
                  <span style={{ background: '#f59e0b', color: '#0f172a', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold' }}>
                    {inmueble.etapa} • Entrega {inmueble.entrega}
                  </span>
                )}
              </div>
              <h1 className="prop-h1" style={{ fontSize: '30px', fontWeight: '800', color: '#0f172a', margin: '0 0 8px 0', lineHeight: '1.25' }}>
                {inmueble.titulo}
              </h1>
              <p style={{ fontSize: '15px', color: '#64748b', margin: 0, display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                📍 {inmueble.direccion} • <strong>{inmueble.ciudad}, {inmueble.departamento}</strong>
              </p>
            </div>

            <div className="prop-price-box" style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '12px', color: '#64748b', textTransform: 'uppercase', fontWeight: 'bold', letterSpacing: '0.05em' }}>Precio de Venta</div>
              <div className="prop-price-val" style={{ fontSize: '34px', fontWeight: '900', color: '#0284c7' }}>
                {formatCurrency(inmueble.precio)}
              </div>
              {inmueble.precio_texto && (
                <div style={{ fontSize: '13px', color: '#64748b' }}>{inmueble.precio_texto}</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="prop-main-grid">
          ...
          <div>
            ...
            <div className="prop-media-stage">
              ...
              <div style={{ position: 'absolute', top: '14px', left: '14px', background: 'rgba(15,23,42,0.85)', backdropFilter: 'blur(6px)', color: 'white', padding: '5px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold' }}>
            ...
            {mediaList.length > 1 && (
              <div style={{ display: 'flex', gap: '10px', overflowX: 'auto', padding: '12px 2px', scrollbarWidth: 'thin' }}>
                {mediaList.map((media, idx) => (
                  <button
                    key={idx}
                    onClick={() => setActiveMediaIndex(idx)}
                    style={{
                      border: activeMediaIndex === idx ? '3px solid #0284c7' : '2px solid transparent',
                      borderRadius: '8px',
                      overflow: 'hidden',
                      flexShrink: 0,
                      width: '80px',
            ...
            <div className="prop-card-padding" style={{ background: 'white', padding: '28px', borderRadius: '16px', border: '1px solid #e2e8f0', marginBottom: '24px' }}>
              <h2 style={{ fontSize: '22px', fontWeight: '800', color: '#0f172a', marginTop: 0, marginBottom: '14px' }}>
          ...
          <div>
            <div className="prop-sidebar-sticky">
              ...
              {/* CARD DE CONTACTO CON ASESORA ANA */}
              <div style={{ 
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', paddingBottom: '14px', borderBottom: '1px solid #f1f5f9' }}>
                  <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: '#f59e0b', color: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', fontWeight: 'bold' }}>
                    👩‍💼
                  </div>
                  <div>
                    <div style={{ fontWeight: 'bold', fontSize: '16px', color: '#0f172a' }}>Ana</div>
                    <div style={{ fontSize: '13px', color: '#64748b' }}>Asesora Inmobiliaria Kalarti</div>
                  </div>
                </div>

                <div style={{ fontSize: '13px', color: '#64748b', fontWeight: 'bold', textTransform: 'uppercase' }}>Precio de Venta</div>
                <div style={{ fontSize: '26px', fontWeight: '900', color: '#0284c7', margin: '4px 0 16px 0' }}>
                  {formatCurrency(inmueble.precio)}
                </div>

                {/* BOTON WHATSAPP CON ANA */}
                <a 
                  href={whatsappUrl} 
                  target="_blank" 
                  rel="noopener noreferrer"
                  style={{
                    ...
                    background: '#22c55e', color: 'white', padding: '15px', borderRadius: '12px',
                    textDecoration: 'none', fontWeight: '800', fontSize: '16px', marginBottom: '16px',
                    boxShadow: '0 8px 16px -4px rgba(34, 197, 94, 0.4)', transition: 'transform 0.2s'
                  }}
                  onMouseOver={e => e.currentTarget.style.transform = 'translateY(-2px)'}
                  onMouseOut={e => e.currentTarget.style.transform = 'none'}
                >
                  <span style={{ fontSize: '20px' }}>💬</span> Escribir a Ana por WhatsApp
                </a>
                ...
                {formStatus.success ? (
                  <div style={{ background: '#ecfdf5', color: '#065f46', padding: '16px', borderRadius: '12px', textAlign: 'center', border: '1px solid #a7f3d0' }}>
                    ...
                    <div style={{ fontWeight: 'bold', fontSize: '15px', marginBottom: '4px' }}>¡Solicitud enviada a Ana!</div>
                  ...
                  <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: '#334155', marginBottom: '4px' }}>Consulta para Ana</label>
                      <textarea 
                        rows="3" 
                    ...
                    <button 
                      ...
                      style={{ 
                        background: '#0f172a', color: 'white', padding: '13px', borderRadius: '10px', 
                        fontSize: '14px', fontWeight: '700', border: 'none', cursor: 'pointer',
                        boxShadow: '0 4px 6px rgba(15,23,42,0.2)'
                      }}
                    >
                      {formStatus.loading ? 'Enviando a Ana...' : 'Enviar Consulta a Ana'}
                    </button>
                  </form>
                )}
              </div>
      ...
      <div className="mobile-floating-bar">
        ...
        <a 
          ...
          rel="noopener noreferrer"
          style={{
            background: '#22c55e', color: 'white', padding: '12px 18px', borderRadius: '10px',
            textDecoration: 'none', fontWeight: '800', fontSize: '14px', display: 'flex', alignItems: 'center', gap: '6px'
          }}
        >
          <span>💬</span> Chatear con Ana
        </a>
      </div>
